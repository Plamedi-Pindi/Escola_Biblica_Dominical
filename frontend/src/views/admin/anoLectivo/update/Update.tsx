...
type Props = {
    closeModal: () => void,
    anoLetivo: AnoLectivoType
}

const UpdateYearPage = ({ closeModal, anoLetivo }: Props) => {
    const [formData, setFormData] = useState<AnoLectivoType>(anoLetivo);
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await MainAPI.put(`/anoLectivo/update/${id}`, formData)
            closeModal()
            setAnoLectivos(prev =>
                prev.map(ano =>
                    ano._id === id ? response.data : ano
                )
            )
            return response
        } catch (error) {
            console.error("Error ao atualizar ano lectivo", error);
        }
    }
    ...
    return (
        <CustomContainer style="bg-white p-4 rounded-lg absolute top-[50%] left-[50%] -translate-[50%] w-[95%] ">
            <Typography className="!text-lg !mb-2 !font-bold">Atualizar Ano Lectivo</Typography>
            <CustomForm onSubmit={handleSubmit} >
                ...
                <div className="flex items-center justify-between  mb-10 w-full ">
                    ...
                    <div>
                        ...
                        <TextField
                            type="date"
                            value={formData.initDate && formData.initDate.split("T")[0]}
                            variant="standard"
                ...
                <div className="flex justify-end gap-4 pb-4  ">
                    ...
                    <Button variant="contained" className="!text-sm" type="submit" disabled={isDisable}>Atualizar</Button>
                </div>
            </CustomForm>
