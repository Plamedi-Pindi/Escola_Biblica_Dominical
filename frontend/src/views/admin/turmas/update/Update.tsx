...
const UpdateClassPage = ({ closeModal, turma }: Props) => {
    const [formData, setFormData] = useState<TurmaType>(turma); // Estado do formulario
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await MainAPI.put(`/turma/update/${formData._id}`, formData)
            if (response) setTurmas(prev =>
                prev.map(turma =>
                    turma._id === formData._id ? response.data : turma
                )
            )
            closeModal()
    ...
    return (
        <CustomContainer style="bg-white p-4 rounded-lg absolute top-[50%] left-[50%] -translate-[50%] w-[95%] ">
            <Typography className="!text-lg !mb-2 !font-bold">Atualizar Turma</Typography>
